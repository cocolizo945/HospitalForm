...
RUN set -eux; \
  ...
  curl --silent --fail --location --retry 3 --output /usr/local/bin/install-php-extensions \
  ...
  # Install necessary/useful extensions not included in the base image
  ...
  curl --silent --fail --location --retry 3 --output /tmp/keys.dev.pub \
  ...
  curl --silent --fail --location --retry 3 --output /tmp/keys.tags.pub \
  ...
  curl --silent --fail --location --retry 3 --output /tmp/installer.php \
...
# This is defined as the last target to be backward compatible with builds without an explicit --target option
